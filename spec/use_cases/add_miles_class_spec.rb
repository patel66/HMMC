...
describe HMMC::AddMilesClass do
  ...
  it "can't add 100 miles to a classroom in one input" do
   ...
   result = subject.run({:classroom=> "History", :milesclass=> 101, :school_id => school.id })
   # binding.pry
  ...
  it "adds miles to class, also shows up in school" do
   ...
   # binding.pry
   result = subject.run(:classroom => "History", :school => school, :milesclass => 65, :school_id => school.id)
   ...
   expect(result.school.total_miles).to eq 65
